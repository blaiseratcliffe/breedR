...
The profile is flat near its peak.
With nine founders per population $\lambda$ is weakly identified, and the best grid point shifts from one simulated data set to the next; the simulated value is $\lambda = 2/3$.
The estimate is only as fine as the grid, which extends past 1 so that a peak above 1 would show up inside it rather than at the edge.
...
Analogously, if I include only the hybrid subpopulation, it works quite well, identifying the base variance.
