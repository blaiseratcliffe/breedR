...
The two variances enter the covariance of the mixture linearly, once the relationships are split
by the population they come through.
Following García-Cortés and Toro (2006), write $A = A_E + A_J$, where $A_E$ ($A_J$) accumulates
only the relationships passing through founders of population E (J), together with that
population's share of each Mendelian sampling term.
Splitting the matrix in blocks corresponding to each subpopulation, the covariance matrix for the
mixture is
\begin{equation}
  \begin{split}
    \Sigma & =
    \begin{bmatrix}
      \sigma^2_E A_{E:E}  & \mathbf{0}        & \sigma^2_E A_{E:EE} & \sigma^2_E A_{E:H} & \mathbf{0} \\
      \mathbf{0}          & \sigma^2_J A_{J:J} & \mathbf{0} & \sigma^2_J A_{J:H} & \sigma^2_J A_{J:JJ} \\
      \sigma^2_E A_{E:EE} & \mathbf{0}        & \sigma^2_E A_{EE} & \sigma^2_E A_{EH} & \mathbf{0} \\
      \sigma^2_E A_{E:H}  & \sigma^2_J A_{J:H} & \sigma^2_E A_{HE} & \sigma^2_E A^E_{HH} + \sigma^2_J A^J_{HH} & \sigma^2_J A_{HJ} \\
      \mathbf{0}          & \sigma^2_J A_{J:JJ} & \mathbf{0} & \sigma^2_J A_{JH} & \sigma^2_J A_{JJ} \\
    \end{bmatrix} \\
    & = \sigma^2_E \left( A_{E} + \lambda A_{J} \right),
  \end{split}
\end{equation}
where $\lambda = \frac{\sigma^2_J}{\sigma^2_E}$, and $A^E_{HH}$, $A^J_{HH}$ are the parts of the
hybrid-hybrid relationships passing through E and through J founders.
Every relationship between a hybrid and a pure E individual passes through E founders, so those
blocks keep the factor one, and the pure J ones carry $\lambda$.
Within the hybrids no single factor will do: full sibs give $\frac{1 + \lambda}{2} A_{HH}$, but
half sibs are related through E or through J, depending on which parent they share.

$\Sigma$ is linear in $\sigma^2_E$ and $\sigma^2_J$, so both could be estimated at once with two
effects having covariance matrices $A_E$ and $A_J$.
Here we take the simpler route of profiling the ratio $\lambda$, which needs a single variance
component per fit.
...
The log-likelihood peaks inside the grid, and the fit below uses the best grid point.
The estimate is only as fine as the grid, and if the best point lands on either end, the grid needs widening.
With nine founders per population $\lambda$ is estimated imprecisely.
The same simulation run under R's pre-3.6 sampler (`RNGkind(sample.kind = "Rounding")`) puts the best grid point at 0.6 rather than `r lambda[which.max(lik)]`.
The simulated value is $\lambda = 2/3$.

The profile uses only the pure offspring, whose blocks reduce to $A_{EE}$ and $\lambda A_{JJ}$.
At the simulated $\lambda$ the scaled matrix matches the covariance of the simulated breeding
values exactly, hybrids included.

The hybrids are left out of the fits, but they do carry information on both variances: hybrids
that share their E parent are related through $\sigma^2_E$, and those that share their J parent
through $\sigma^2_J$.
...
The best grid point is $\lambda$ = `r lambda0`, giving an estimated additive-genetic variance for the `J` population of `r round(res3$var['E', 1]*lambda0,2)`, against a simulated value of `r sigma2['J']`.
