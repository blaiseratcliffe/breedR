...
The profile uses only the pure offspring.
For them, the scaled matrix at the simulated $\lambda$ matches the covariance of the simulated breeding values exactly.
The scaling does not extend to the hybrids: for $\lambda$ below about 0.63 the offspring block of the scaled matrix is not positive definite, so it is not a valid covariance matrix.

Fitting the hybrids alone estimates their additive-genetic variance, $(\sigma^2_E + \sigma^2_J)/2$, but cannot separate the two components.
Every hybrid-hybrid factor is $(1 + \lambda)/2$, so the hybrids inform only $\sigma^2_E (1 + \lambda)/2$.
